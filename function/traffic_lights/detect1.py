...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            init_image=cv2.resize(im0,(320,240),interpolation=cv2.INTER_CUBIC)#修改图片和视频检测时输出的窗口大小
            #cv2.imshow('start', init_image)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        BIAOQIAN=label[0:3]                      
                        if BIAOQIAN=="red" and area >= 30000:  #红灯制动
                           SetDutyCycle_Values = -0.015
                           SetMode = SetDutyCycle(SetDutyCycle_Values)
                           get_values_example(SetMode)
                        else:
                           SetDutyCycle_Values = 0.06
            ...
            # 车道保持           
            gray = LKS.grayscale(init_image)
            blur_gray = LKS.gaussian_blur(gray)
            ...
            if view_img:
                im0=cv2.resize(im0,(640,480),interpolation=cv2.INTER_CUBIC)#修改图片和视频检测时输出的窗口大小
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / '/home/rtech/yolov5-6/best.pt', help='model path(s)')
