...
# 地址码：0x55 0xA5
# 数据长度：0x0A，知从“数据长度”到“雷达关闭提示”的总字节数
# 指令码：0xD3，指查询微波检测信息
...
def openMMWPort(uart=MMWR_PORT, baudRate=MMWR_BAUD_RATE) -> serial.Serial:
    """
    打开串口
    ...
    """
    try:
        uart = serial.Serial(uart, baudRate, timeout=5)
...
def checkData(bytesData: bytes) -> bool:
    ...
    return True if bytesData[-1] == (checkSum - bytesData[-1]) else False


def analyseData(bytesData: bytes) -> int and int:
    """
    解析数据
...
def MMWDetection(uart: serial.Serial) -> int and int:
    """
    毫米波检测函数，需放置在循环中执行
...
if __name__ == "__main__":
    Uart = openMMWPort(MMWR_PORT, MMWR_BAUD_RATE)  # 打开串口
    while True:
        MMWDetection(Uart)
